window.onload = function() {
    ...
    const integrateRct = (fn, step, start, end) => {
      ...
      return `Regla del rectangulo: ${roundResult(sumRct)}`
    }
    ...
    const integrateTrp = (fn, step, start, end) => {
      ...
      return `Regla del trapecio: ${roundResult(sumTrp)}`
    }
    ...
    btn.addEventListener('click', () => {
      ...
      try {
        ...
        switch(true) {
          case err.message == 'Tipo de argumento inesperado (char 8)' || err.message == 'Valor inesperado (char 8)':
            errorMessage.innerHTML = 'Debes ingresar una integral!';
            cbInput.classList.add('on-error');
            break;
          case err.message.startsWith('Undefined symbol') || err.message.startsWith('Tipo de argumento inesperado'):
            errorMessage.innerHTML = 'Intergral no valida!';
            cbInput.classList.add('on-error');
            break;
          case err.message == 'Range is not valid':
            errorMessage.innerHTML = `Expresion fuera del rango!`;
          ...
          case err.message.includes(','):
            ...
            cbInput.classList.add('on-error');
            break;
          case err.message == 'paso no valido':
            errorMessage.innerHTML = 'El paso no debe ser mayor que el rango!';
